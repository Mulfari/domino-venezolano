...
function EndBadge({ value, label }: { value: number; label: string }) {
  return (
    <motion.div
      key={value}
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          boxShadow: "0 2px 8px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.6)",
        }}
      >
        <PipGrid value={value} />
      </div>
...
export function BoardEnds() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <EndBadge value={board.left} label="Izq" />
        ...
        <EndBadge value={board.right} label="Der" />
      </motion.div>
    </AnimatePresence>
  );
}
